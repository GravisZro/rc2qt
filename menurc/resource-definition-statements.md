...
| [**DLGINIT**](dlginit-resource.md)           | Defines control initialization data for a dialog box. Windows uses this resource to feed initial settings to controls before the dialog is displayed.                           |
...
| [**TEXTINCLUDE**](textinclude-resource.md)   | Defines metadata used by the Visual C++ Resource Editor to reconstruct the resource script.                      |
...
For the numeric values of predefined Windows constants used in resource scripts (such as window styles, virtual key codes, and control messages), see [**Predefined Constants (WinUser.h)**](winuserh-predefined-constants.md).
