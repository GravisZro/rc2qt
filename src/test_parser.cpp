...
static void test_multiple_controls()
{
  try
  {
    ...
    /* Controls are sorted by x first, then y (left-to-right, then top-to-bottom),
       so the parse order here is the x=7 column (NAME_LABEL, EMAIL_LABEL, OK),
       then the x=42 column (NAME, EMAIL edits), then Cancel at x=62. */
...
static void test_numeric_expressions()
{
  try
  {
    ...
    assert(ud.items[6].value == "0x029a");
    assert(ud.items[7].value == "0o733");
    record_result("numeric_expressions", true);
  }
  catch (const std::exception& e)
  {
    record_result("numeric_expressions", false, e.what());
  }
}

static void test_numeric_token_types()
{
  try
  {
    ...
    {
      if (tok.type == rc::token_type::comma || tok.type == rc::token_type::newline ||
          tok.type == rc::token_type::eof)
        continue;
    ...
    assert(values[3].value == "0x029a");
    assert(values[4].type == rc::token_type::integer_literal);
...
static void test_expression_evaluation()
{
  try
  {
    ...
    assert(evaluate_expression("1 + 2 | 4") == 7);
    assert(evaluate_expression("0x1f & 0x0f") == 15);
    assert(evaluate_expression("((1+5)&1)") == 0);
    assert(evaluate_expression("1024") == 1024);
    assert(evaluate_expression("0o733") == 475);
    assert(evaluate_expression("0o733 + 1") == 476);
    ...
    assert(safe_stoi("0x10") == 16);
    assert(safe_stoi("0o733") == 475);
    assert(safe_stoi("010") == 10);
