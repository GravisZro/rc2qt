...
int int_child(const pugi::xml_node& node, const char* tag)
{
  pugi::xml_node c = node.child(tag);
  return c ? c.text().as_int(0) : 0;
}

std::string text_child(const pugi::xml_node& node, const char* tag)
{
  pugi::xml_node c = node.child(tag);
  return c ? c.text().get() : "";
}

void apply_size(QWidget* w, const pugi::xml_node& prop)
{
  pugi::xml_node s = prop.child("size");
  if(!s)
    return;
...
void apply_sizepolicy(QWidget* w, const pugi::xml_node& prop)
{
  pugi::xml_node sp = prop.child("sizepolicy");
  if(!sp)
    return;
  QSizePolicy policy(
...
void apply_font(QWidget* w, const pugi::xml_node& font)
{
  QFont f = w->font();
  ...
  if(pugi::xml_node b = font.child("bold"))
    f.setBold(b.text().as_bool(false));
  if(pugi::xml_node i = font.child("italic"))
    f.setItalic(i.text().as_bool(false));
  w->setFont(f);
}

void apply_widget_properties(QWidget* w, const pugi::xml_node& node)
{
  for(pugi::xml_node prop : node.children("property"))
  {
    std::string name = prop.attribute("name").value();
    pugi::xml_node value = prop.first_child();
    if(!value)
      continue;
...
void apply_layout_properties(QLayout* layout, const pugi::xml_node& node)
{
  for(pugi::xml_node prop : node.children("property"))
  {
    std::string name = prop.attribute("name").value();
                  ...
                  Qt::Alignment item_align = Qt::Alignment())
{
  ...
  for(pugi::xml_node item : node.children("item"))
  {
    ...
    else if(child_tag == "spacer")
    {
      ...
      int sw = 20;
      int sh = 20;
      pugi::xml_node prop = child.find_child_by_attribute("property", "name", "sizeHint");
      if(pugi::xml_node s = prop.child("size"))
      {
...
QWidget* build_widget(const pugi::xml_node& node, QWidget* parent, builder& b)
{
  ...
  if(pugi::xml_node layout = node.child("layout"))
    build_layout(layout, w, b, nullptr, 0, 0, 1, 1);

  if(auto tw = qobject_cast<QTabWidget*>(w))
  {
    for(pugi::xml_node page : node.children("widget"))
    {
      QString title;
      if(pugi::xml_node attr = page.child("attribute");
         attr && std::string(attr.attribute("name").value()) == "title")
        title = QString::fromUtf8(attr.child("string").text().get());
...
result verify_layout(const verify_input& input, const std::string& dump_dir)
{
  ...
  pugi::xml_node ui = input.doc.child("ui");
  pugi::xml_node root_node = ui.child("widget");
  if(!root_node)
    return res;
